...
def get_accepted_sols(param_link,lang_req):
    total_no_of_pages = 0
    # print("link in header = ","https://codeforces.com"+param_link+"/page/1" )
    result = requests.get("https://codeforces.com"+param_link+"/page/1" , headers=headers ,verify=False)
    ...
    while flag:
        try:
            ...
            if(is_page_loaded(result,"get_accepted_sols with link "+"https://codeforces.com"+param_link+"/page/"+str(page_number_of_sol))):
                return None
            log_file("url for sol = ","https://codeforces.com"+param_link+"/page/"+str(page_number_of_sol))
            ...
            for row in rows:
                try:
                    ...
                    if(len(tlist)==8):
                        ...
                        sol_id  = tlist[0].text
                        sol_id = sol_id.lstrip()
                        sol_id = sol_id.rstrip()
                        # print(" sol_id ",sol_id)
                        if(links == None):
                        ...
                        if(lang_req=="python" and sol_lang.find("ython")!=-1):
                            ...
                            c_flag = c_flag + 1
                            # print("sol_id here = ",str(sol_id))
                            ret_obj["sol_id"] = str(sol_id)
                            # print(" ret_obj[sol_id] ",ret_obj["sol_id"])
                            ret_obj["sol"] = code
                            ...
                            ret_obj["time_taken"] = sol_time_taken
                            ret_obj_list.append(ret_obj)
                        if(ret_obj["sol"] != "no code found"):
                            no_of_sol_captured += 1
                        # print(ret_obj)
                        # print()
                        # print("no_of_sol_captured ",no_of_sol_captured)
                        if(no_of_sol_captured ==10):
                            return ret_obj_list
                    ...
                    print("exception occured while fetchong 10 sols ")
                    no_of_sol_captured = len(ret_obj_list)
                    print()
                    print("no_of_sol_captured in exception len = ",no_of_sol_captured)

            if(no_of_sols_for_a_question<50):
                flag = False
                break
        except:
            print("exception while loading page of diff page of solution")
            ...
            try:
                ...
                if(len(tlist)==5 and problem_tag!=""):
                    ...
                    if(input_p_id==problem_id):
                        start = True
                    elif(input_p_id==""): #for resume
                        start = True
                    if(start):
                        # print("problem id ",problem_id)
                        obj_returned_list = get_accepted_sols(sol_link,"c++")
                        # print("received list")
                        # print()
                        ...
                        if(difficulty_value=="" or difficulty_value==None):
                            difficulty_value = ""
                        if(len(obj_returned_list) == 0):
                            missed_problem_ids.append(problem_id)
                            log_file("missed problem id ",problem_id)
                            continue
                        for obj_returned in obj_returned_list:
                            ...
                            csv_row.append(obj_returned["sol_id"])
...
if(input_page_no==0 and input_p_id=="1144G"):
    csv_row = ["id","name","problem statement","tags","difficulty","sol id","solution","time_taken","author"]
    with open('v2_tagged_codeforces.csv', 'a') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerow(csv_row)
    csvFile.close()
