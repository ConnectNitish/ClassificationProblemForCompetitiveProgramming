...
# filenameList = ["questions_ALEXNUMB.csv"]
...
for item in filenameList:
	...
	for row in rows:
		...
		#if row[4] == 'accepted' and row[0] not in data_of_solution_details.keys() and testValue < 7:
		...
		if row[4] == 'accepted':
			...
			if row[0] not in data_of_solution_details.keys():
				data_of_solution_details[row[0]] = [{row[1]:[row[2],row[3],row[4],row[5],row[6],row[7],row[8]]}]
				#print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
				#print({row[0]:{row[1]:[row[2],row[3],row[4],row[5],row[6],row[7],row[8]]}})
			else:
				get_temp_list = data_of_solution_details[row[0]]
				get_temp_list.append({row[1]:[row[2],row[3],row[4],row[5],row[6],row[7],row[8]]});
				data_of_solution_details[row[0]] = get_temp_list 
			#if(testValue>7):
			#	break;			
			#testValue = testValue + 1
		
		# if len(data_of_solution_details)>50:
		# 	break
...
# print(question_submission_id_key_list)
# print(data_of_solution_details.keys())
...
fields = ['QuestionCode','Title','Questionlink','Difficultylevel','Problem Statement','Editorial','Tags','Time Limit','Languages','Solution','SubmissionID']
...
for item in data_of_solution_question:
	...
	if question_code in question_code_accepted_key_list:
		...
		'''print("------------&&&&&&&&&&&&&&&&&&&&---------------------------")
		print(question_code)
		print(value_of_accepted_solution)
		break'''
		for value_of_accepted_solution_temp in value_of_accepted_solution:

			# if len(final_result_to_put_in_file) > 5000:
			# 	break

			#timeago,UserID,Status,TimeTaken,MemTaken,Language,SolutionUrl
			submission_id_of_accepted_solution_list = value_of_accepted_solution_temp.keys()
			submission_id_of_accepted_solution_list_values = value_of_accepted_solution_temp.values()
			'''print("--------------------------------------------------------")
			print(question_code)
			print(submission_id_of_accepted_solution_list[0])
			print(submission_id_of_accepted_solution_list_values[0])'''

			if submission_id_of_accepted_solution_list != None:
				'''print("--------------------------------------------------------")
				print(submission_id_of_accepted_solution_list[0])
				print(submission_id_of_accepted_solution_list_values[0])
				print(item)
				...
				if submission_id_of_accepted_solution in question_submission_id_key_list:
					...
					temp_Title = item[1]
					temp_Submission_ID = submission_id_of_accepted_solution

					#print(temp_question_code,temp_Title,temp_question_link,temp_Level,temp_problem_statement,temp_Editorial,temp_Tags,temp_time_taken,temp_language,temp_actualSolutionText)	
					if len(final_result_to_put_in_file) % 500 == 0:
						print(len(final_result_to_put_in_file))
					final_result_to_put_in_file.append([temp_question_code,temp_Title,temp_question_link,temp_Level,temp_problem_statement,temp_Editorial,temp_Tags,temp_time_taken,temp_language,temp_actualSolutionText,temp_Submission_ID])
					# break
...
# name of csv file 
filename = "AllDetails_WithSubmIssionID.csv"
